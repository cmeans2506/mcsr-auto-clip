...
from rsg.paceman_service import LiveRunData, Event, PacemanService
import util
...
class CoverGenerator(BaseCoverGenerator):
    ...
    def generate_event_str_list(self) -> list[str]:
        def get_event_str(event: Event):
            event_str = f"{util.ts_to_str_sec(event.igt)}{event.eventId.abbr}"
    ...
    def get_bg_path(self) -> Path:
        return config.rsg_video_dir / f'bg[{self._live_run.id}].webp'
    ...
    def get_render_data(self) -> dict[str, Any]:
        ...
        return{
            ...
            'is_complete_run': self._live_run.is_complete_run,
        }

    def get_save_path(self) -> Path:
        return config.rsg_video_dir / f'cover[{self._live_run.id}].jpg'

    def get_html_path(self) -> Path:
        return config.rsg_video_dir / f'cover[{self._live_run.id}].html'
