...
这是一个用于自动切片1.16rsg和ranked的脚本， 符合条件就可以自动切片上传，省去后期的时间。
...
双击 `main` 文件夹下的 `main.exe`，填写游戏名称，修改视频文件夹（剪辑成片输出的位置） ，点击启动，开启脚本。
...
- 如果启用了上传功能，脚本会让你登录一个bilibili账号，这个账号是你用于上传视频的账号，推荐**扫码登录**。
...
#### ②安装相关依赖
- 双击 `./src/install-requirements.bat` ，脚本会自动安装python包
- 在 `src` 同目录下，需要包含一个 `assets` 文件夹，其中的内容包含：
...
| `rsg.rsg.enter_nether`           | Number  | rsg进下界最大允许时长（毫秒）                                                         |
...
| `use_upload`                     | Boolean | 是否启用上传至哔哩哔哩                                                              |
...
| `death_clip_ahead_seconds`       | Number  | 切片起始点的提前时间量                                                              |
 ...
 - 其中，`upload_setting` 中的最大允许时长一定是小于等于 `clip_setting` 中的最大允许时长，否则没有意义

---

 - `./scripts/concat.bat` 是用于合并所有死亡切片的脚本，在 `death_clip` 文件夹下，双击运行，会产生`output.mp4`

---

 `pb.json` 文件：
存储你的pb信息，该文件位于 `C:\Users\<用户名>\AppData\Roaming\mcsr auto clip` 文件夹中
```json
{
   "rsg.first_portal": {
        "id": 0,
...
项目启动后。会在目录中产生
- `logs/` ，用于脚本输出日志文件
- `rsg/` ，存储所有 1.16 any% 速通视频。视频会根据日期进行分类。`world[<世界ID>].mp4`
- `ranked/` ，存储所有 ranked 速通比赛视频。视频会根据日期进行分类。`match[<比赛ID>].mp4`
- `death_clip/` ，用于存储所有死亡切片。视频会根据日期进行分类。产生`match[<比赛ID>]<时间节点>.mp4`文件。同时同目录下还会产生`filelist.txt`文件，这是用于合并所有切片用到的文件。
